...
import { unicodeRequests } from "../requests/unicode.requests.js";
...
const runProviderFallback = async () => {
    ...
    const resultsPath = path.join(resultsDir, "unicode.results.json");

    await access(resultsPath);

    const resultContent: TestResult[] = [];
    for (const testCase of unicodeRequests) {
