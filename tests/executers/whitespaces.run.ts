...
import { whitespacesRequests } from "../requests/whitespaces.requests.js";
...
const runProviderFallback = async () => {
    ...
    const resultsPath = path.join(resultsDir, "whitespaces.results.json");

    await access(resultsPath);

    const resultContent: TestResult[] = [];
    for (const testCase of whitespacesRequests) {
